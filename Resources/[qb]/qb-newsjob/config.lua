...
-- ============================================================
-- 2026-09-10 経済設計: $150 → $830
-- 記者は「客/取材対象との会話」というRP的見返りがあるため、単調・拘束型($7,500/h)ではなく
-- 社交型として目標$5,000/h に置いた。取材1件を約10分と仮定して $830 とした。
-- ※取材1件の所要時間はコードから読めないため暫定値。
Config.NewsReward = 830 -- 取材1件を完了した際の報酬
...
Config.NewsLocations = {
    ...
    { coords = vector3(440.81, -981.33, 30.69), label = 'ミッションロウ署前' }, -- [2026-09-07 修正] 実機で/coords確認した実座標に更新(旧座標との差は1m未満)
    { coords = vector3(-1850.99, -1232.4, 13.02), label = 'デルペロ埠頭' },
    { coords = vector3(-1197.68, -1531.85, 4.36), label = 'ヴェスパッチ・ビーチ' },
    { coords = vector3(732.56, 1200.1, 326.18), label = 'ヴィンウッドサイン展望地' }, -- [2026-09-07 修正] 実機で/coords確認した実座標に更新(旧座標はZが約33mズレていた)
}
