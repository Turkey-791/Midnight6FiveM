Config = {}
-- ============================================================================
-- 2026-09-12 UI統一(最終切替): ラジアルメニューの「UI」は ao_radial(ox_lib)へ移管した。
-- このリソースは、ドア開閉 / 座席移動 / 車両エクストラ / 服装トグル / トランク /
-- ストレッチャー などの「実処理」を持つハンドラ置き場として残す。
--
-- ★ 停止してはいけない。停止するとハンドラごと消えて ao_radial 側も動かなくなる。
--
-- 下のフラグを false に戻すと、この qb-radialmenu のUIが再び開くようになる(1行で復帰)。
-- ============================================================================
...
Config.JobInteractions = {
    ...
    -- [2026-09-06 追加] ニュースレポーター(reporter)用。/newsassignment
    -- コマンドと同じ処理を、このWorkメニューからも実行できるようにする。
    -- (処理の実体はqb-newsjob/server/main.luaのRequestNewsAssignment関数)
    ['reporter'] = {
        {
            id = 'newsassignment',
            title = '取材依頼を受ける',
            icon = 'newspaper',
            type = 'server',
            event = 'qb-newsjob:server:requestAssignment',
            shouldClose = true
        }
    }
}
